...
public static class MediaResolver
{
    ...
    public static async Task<ResolvedQuery> ResolveAsync(string url, CancellationToken ct)
    {
        ...
        if (root.TryGetProperty("entries", out var entries) && entries.ValueKind == JsonValueKind.Array)
        {
            ...
            if (items.Count == 0)
                throw new YtDlpException("No videos found in this link. Try a direct video or playlist link.");
    ...
    private static List<QualityOption> BuildQualityOptions(JsonElement root)
    {
        ...
        var videoSizes = new Dictionary<int, long>(); // height -> best size
        ...
        foreach (var (label, height) in Tiers)
        {
            ...
            {
                Label = height == 0 ? "Best quality" : label,
                FormatArg = height == 0
    ...
    public static QualityOption QualityFromLabel(string? label)
    {
        if (!string.IsNullOrWhiteSpace(label))
        {
            foreach (var (l, height) in Tiers)
            {
        ...
        return new QualityOption { Label = "Best quality", FormatArg = "bestvideo+bestaudio/best" };
    }

    public static List<QualityOption> BuildDefaultTiers() =>
        Tiers.Select(t => new QualityOption
        {
            Label = t.Label,
            FormatArg = t.Height == 0
                ? "bestvideo+bestaudio/best"
                : $"bestvideo[height<={t.Height}]+bestaudio/best[height<={t.Height}]/best"
        }).ToList();
